...
* Names may not end with an underscore.
...
The following prefixes have a universal meaning ensuring the columns can be read
and understood easily from SQL code. Use the correct prefix where appropriate.

* `__pk`—a unique identifier for primary key.
* `_fk`—a unique identifier for foreign key.
* `z`—a unique identifier for information generated by the database.
  * `a`—a unique identifier for auto entered.
  * `c`—a unique identifier for a calculation.
    * `t`—a unique identifier for text
    * `n`—a unique identifier for number
    * `d`—a unique identifier for a date
    * `m`—a unique identifier for a timestamp
